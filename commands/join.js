...
async function execute(message, args, user) {
  ...
  if (!(message.channel.id in channels)) {
    return message.channel.send(errorMessage("No game in this channel!"));
  }

  const current_game = await game_info.get(channels[message.channel.id]);

  if (current_game.gameState.phase !== "joinWait") {
    return message.channel.send(errorMessage(current_game.players.map((player) => player.id).includes(message.author.id) ? "Already joined game!" : "Game already started!"));
  }
  ...
  if (!current_game.players.map((player) => player.id).includes(message.author.id)) {
    ...
    if (current_game.players.length === current_game.playerCount) {
      ...
      await assignRoles(current_game, message);
...
async function assignRoles(current_game, message) {
  const roleConfigs = {
  ...
  if (current_game.customGameSettings.monarchist) {
    for (let count in roleConfigs) {
      let roles = roleConfigs[count];
      let index = roles.indexOf("fascist");
  ...
  if (current_game.customGameSettings.avalon) {
    for (let count in roleConfigs) {
      let roles = roleConfigs[count];
      let index = roles.indexOf("liberal");
      ...
      for (let j = 0; j < current_game.playerCount; j++) {
        if (i !== j && (roles[j] === "fascist" || roles[j] === "hitler")) {
          ...
          await sendDM(
            message,
            current_game,
            `The player <@${current_game.players[j].id}> in seat **${j + 1}** is a **fascist**`,
            current_game.players[i].id
          );
        }
      }
      ...
      for (let j = 0; j < current_game.playerCount; j++) {
        if (roles[j] === "fascist" || roles[j] === "monarchist" || roles[j] === "hitler") {
          await sendDM(
            message,
            current_game,
            `The player <@${current_game.players[j].id}> in seat **${j + 1}** is a fascist-aligned role`,
            current_game.players[i].id
          );
        }
      }
