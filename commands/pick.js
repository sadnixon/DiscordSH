...
async function execute(message, args, user) {
  ...
  if (message.channel.id in channels) {
    ...
    let termlocked = [current_game.gameState.presidentId, current_game.gameState.lastChancellorId];
    if (current_game.players.length - current_game.gameState.deadPlayers.length > 5) {
        termlocked.push(current_game.gameState.lastPresidentId);
    }
    if (
      args &&
      _.range(0,7).includes(parseInt(args[0])) &&
